...
- **Traffic controls**: the key set is cached until the `Cache-Control: max-age` it was served with. A single `jwksMinRefreshInterval` floor (1 minute) governs *every* reason to refetch — a stale set, an unknown `kid`, or recovery from a failed fetch — and `lastFetch` is stamped before the request is attempted, so failures throttle exactly as successes do. Both uncovered paths are amplifiers: `kid` is attacker-controlled, and a failed fetch never advances `staleAt`, so a Google outage would otherwise leave every sign-in retrying immediately.
...
- **`www.` is normalised away on both sides**: the apex and the `www` host are one deployment — the edge answers the apex with a `308` to `www` — so a destination spelled either way lands back on this service. Comparing raw hostnames let whichever spelling `BASE_URL` does not carry slip through, and a self-referential link reached production that way. `canonicalHost` lowercases and drops a leading `www.` from both the configured host and the candidate.
- **It is a host equality check, not a substring match**: `https://trimto.me.evil.com`, `https://sub.trimto.me` and `https://example.com/trimto.me` all remain shortenable. Only the deployment's own host, in either spelling, is refused — `www` is the single normalisation, not a general subdomain rule.
...
| `internal/service` (ID token verification) | Genuine RS256 tokens signed against an httptest JWKS: `alg: none`, HS256 confusion, RS512 downgrade, expired, missing `exp`, not-yet-valid, unknown `kid`, right `kid` with the wrong key, and malformed input are each rejected |
| `internal/service` (JWKS cache) | The set is fetched once and reused; forged kids do not each trigger a refresh; a failing endpoint is not retried on every sign-in; a cached key still verifies after the set goes stale; a rotated key is adopted once the throttle elapses; a key advertising a non-RS256 algorithm is not admitted |
...
| `internal/service` (self-reference) | Destinations on this deployment's own host are refused across scheme, case, port and `www.` variants and never reach the repository, in both directions (`www` destination against an apex `BASE_URL` and the reverse); look-alike hosts (`sub.`, `.evil.com`, path mentions) stay shortenable; an unset `BASE_URL` disables the check |
